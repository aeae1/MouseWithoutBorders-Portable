...
- Opens Mini Log in a readable, selectable, modeless viewer instead of disabling Settings or overwriting the clipboard automatically.
...
15. **Fixed key application and made diagnostics inspectable.** Applying a typed security key now updates both the live encryption state and the adjacent preferences JSON, forces that save to finish before sockets reconnect, and compares keys with case-sensitive semantics. The **Mini Log** link now opens an About-sized modeless window with scrollable, selectable text and an explicit **Copy all** button instead of silently replacing the clipboard. Unlike the Test 10 modal viewer, it does not disable the Settings window; repeated clicks refresh and focus the existing viewer rather than stacking windows. New preference files start with **Wrap mouse** off so an outer matrix edge does not unexpectedly jump to the opposite side; existing saved choices are not migrated or overwritten.
