...
- High-resolution machine tiles: **added in RC7**. The 43×27 monitor bitmaps formerly stretched across each machine tile are replaced by 424×216 active and inactive PNG artwork matched to the display area; the graphite/silver configured state uses restrained emerald, blue, cyan, and violet, while the empty state is grayscale.
...
- Real-Windows connection testing: **working through Test 12**. Version 1.0.0 Release Candidate 7 keeps that behavior and the RC1–RC6 fixes while adding the high-resolution machine-tile artwork; a final portable/self-install, firewall, sleep/wake, shortcut, visual-scaling, and two-machine regression pass remains worthwhile before 1.0.0 stable.
- Machine-tile artwork validation: **pending for RC7**. The replacement assets are four times the logical display resolution and do not touch network, input, preferences, or connection-state logic; they should still be checked at common Windows scaling levels and in both enabled and disabled states on real Windows.
