...
The Computer Matrix uses one double-buffered drawing surface for monitor artwork, status and drag previews. Computer names and checkboxes remain native Windows controls, retaining editing, validation and keyboard focus. Layout finishes before the first displayed frame. Connection statuses are computed before updating their presentation, avoiding repeated clear/reapply cycles.
