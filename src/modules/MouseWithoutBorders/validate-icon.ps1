...
if ($bytes.Length -lt 6) {
    throw "Standalone icon '$Path' is too small to be a valid ICO file."
}
...
if ($reserved -ne 0 -or $imageType -ne 1) {
    throw "Standalone icon '$Path' does not have a valid ICO header."
}

if ($imageCount -ne $expectedSizes.Count -or $bytes.Length -lt (6 + (16 * $imageCount))) {
    throw "Standalone icon '$Path' must contain $($expectedSizes.Count) complete image entries."
}
...
for ($index = 0; $index -lt $imageCount; $index++) {
    ...
    if ($width -ne $height -or $bitsPerPixel -ne 32) {
        throw "Standalone icon entry $index must be a square 32-bit image."
    }

    if ($imageSize -eq 0 -or ([uint64]$imageOffset + [uint64]$imageSize) -gt [uint64]$bytes.Length) {
        throw "Standalone icon entry $index is truncated or points outside the ICO file."
    }
...
if ($sizeDifferences.Count -ne 0) {
    throw "Standalone icon sizes must be: $($expectedSizes -join ', '). Found: $($actualSizes -join ', ')."
}

Write-Host "Standalone icon validated: $($actualSizes -join ', ') pixels ($($bytes.Length) bytes)."
