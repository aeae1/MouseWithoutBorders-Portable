...
The dynamically created **Portable** tab is 563 logical pixels wide in the classic fixed-size settings form. Its explanatory labels use a 505-pixel maximum width, leaving the intended side margins and forcing normal WinForms word wrapping. Do not increase that width beyond the tab's usable client area: DPI scaling otherwise enlarges the label past the panel boundary and clips the sentence instead of wrapping it.
