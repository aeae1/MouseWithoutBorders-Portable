...
The **Mini Log** link remains available in the Settings window as a lightweight support aid. It opens `MiniLogForm`, whose read-only multiline text box supports scrolling, selection, Ctrl+C, and an explicit **Copy all** action. Opening the report must not overwrite the clipboard, and the report must continue showing only the existing checksum representation of the security key rather than the key itself.
