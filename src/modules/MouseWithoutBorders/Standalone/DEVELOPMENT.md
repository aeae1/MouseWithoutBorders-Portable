...
The **Mini Log** link remains available in the Settings window as a lightweight support aid. It opens one reusable modeless `MiniLogForm`; do not use `ShowDialog`, because a modal owned form disables the main Settings window and looks like a hang even though connection and tray processing continue. Its read-only multiline text box supports scrolling, selection, Ctrl+C, and an explicit **Copy all** action. Repeated clicks refresh and activate the existing viewer rather than stacking windows. Opening the report must not overwrite the clipboard, and the report must continue showing only the existing checksum representation of the security key rather than the key itself.
...
Workflow copies of the portable EXE and clean source archive use a one-day retention period. They exist only to carry a tested binary into the release job and support immediate diagnosis; GitHub Release assets are the durable distribution channel. Keep this retention short while rapid test iterations would otherwise accumulate duplicate self-contained runtimes.
